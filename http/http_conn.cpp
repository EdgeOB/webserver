...
const char *error_400_form = "Your request has bad syntax or is inherently impossable to staisfy.\n";
...
void http_conn::initmysql_result(connection_pool *conn_pool) {
    ...
    while(MYSQL_ROW row = mysql_fetch_row(result)) {
        ...
        string temp2(row[1]);
        users[temp1] = temp2;
        LOG_INFO("http_conn::initmysql_result:%s,%s",temp1.c_str(), temp2.c_str());
    }
}
...
//将事件重置修改属性
void modfd(int epfd, int fd, int ev, int TRIGMode) {
    epoll_event event;
    event.data.fd = fd;
    if(1 == TRIGMode) {
...
void http_conn::init(int sockfd, const sockaddr_in &addr, char*root, int TRIGMode, 
    ...
    addfd(m_epfd,m_sockfd,true,TRIGMode);
    m_user_count++;
...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text) {
    m_url = strpbrk(text, " \t");       //find the first position
    if(!m_url) {
        return  BAD_REQUEST;
    }
    *m_url++ = '\0';
    ...
    m_url += strspn(m_url, " \t");  // t连续出现个数
    m_version = strpbrk(m_url, " \t");  //指向第一次出现 t的位置
    if(!m_version) {
        return BAD_REQUEST;
    }
    *m_version++ = '\0';
    ...
    if(strncasecmp(m_url, "http://", 7) == 0) {
        m_url += 7;
        m_url = strchr(m_url, '/');     //找到/第一次出现位置
    }

    if(strncasecmp(m_url, "https://", 8) == 0) {
        m_url += 8;
        m_url = strchr(m_url, '/');     //找第一次出现
    }
...
http_conn::HTTP_CODE http_conn::do_request() {
    ...
    if(cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3')) {
        ...
        int i;
        for(i = 5; m_string[i] != '&' && i - 5 < 99; ++i) {
            name[i - 5] = m_string[i];
        }
        name[i - 5] = '\0';
        int j = 0;
        for(i = i + 10; m_string[i] != '\0' && j < 99; ++i, ++j) {
            passwd[j] = m_string[i];
        }
        passwd[j] = '\0';
    ...
    if(!(m_file_stat.st_mode & S_IROTH)) {  //其他用户没有读取权限
        return FORBIDDEN_REQUEST;
    }
    if(S_ISDIR(m_file_stat.st_mode)) {  //is dir是目录
        return BAD_REQUEST;
    }
...
bool http_conn::write() {
    ...
    if(bytes_to_send == 0) {
        modfd(m_epfd, m_sockfd, EPOLLIN, m_TRIGMode);
        init();
        return true;
    }
    ...
    while(1) {
        ...
        if(temp < 0) {
            if(errno == EAGAIN) {
                modfd(m_epfd, m_sockfd, EPOLLOUT, m_TRIGMode);
                return true;
            }
            unmap();
            return false;
        }
        bytes_have_save += temp;
        bytes_to_send -= temp;

        if(bytes_have_save >= m_iv[0].iov_len) {
            m_iv[0].iov_len = 0;
            m_iv[1].iov_base = m_file_address + (bytes_have_save - m_write_idx);// 剩余未发送内容
            m_iv[1].iov_len = bytes_to_send;
        }
        else {
            m_iv[0].iov_base = m_write_buf + bytes_have_save;
            m_iv[0].iov_len = m_iv[0].iov_len - bytes_have_save;
        }
...
bool http_conn::add_response(const char*format, ...) {
    ...
    int len = vsnprintf(m_write_buf + m_write_idx, WRITE_BUFF_SIZE - m_write_idx - 1,format, arg_list);
    if(len >= (WRITE_BUFF_SIZE - m_write_idx - 1)) {
        va_end(arg_list);
        return false;
    }
    m_write_idx += len;
